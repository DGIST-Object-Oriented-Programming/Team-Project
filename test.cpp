...
void Test::TodayResult(Subject& _sub, Person& _person) {
	...
	if (dayNum == 14) { //기말고사 전날일 때
		...
		std::cout << Subject << " 과목에서 당신의 성취도는 " << _sub.GetAchievement() << "입니다." << std::endl; //성취도 출력
		std::cout << Subject << " 시험의 예상되는 학점은 " << ExpGrad << "입니다." << std::endl; // 예상 학점 출력
	}
	else {
		std::cout << "D-" << dayNum << std::endl; //날짜 출력
		std::cout << "당신의 현재 Level은 " << _person.level << "입니다." << std::endl; //레벨 출력
		std::cout << Subject << " 과목에서 당신의 성취도는 " << GetAchievement() << "입니다." << std::endl; //성취도 출력
	}
}
